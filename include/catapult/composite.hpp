...
struct CompositeFactory {
  ...
  static constexpr auto create(R&& claimSets) {
    ...
    static_assert(composite.isDepthValid(),
  ...
  static constexpr auto create(ClaimSets&&... claimSets) {
    ...
    static_assert(sizeof...(claimSets) > 0,
                  "Composite claim must contain at least one claim set");
    static_assert(sizeof...(claimSets) <= 100,
                  "Too many claim sets in composite");
