...
private:
    ...
    // TODO (Suhas): May be this need not be cache aligned, if we don't care
    // about statistics being off under multi-threaded setup.
...
private:
    ...
    void deallocate_raw(T* ptr) noexcept {
        ...
        const auto* pool_start = reinterpret_cast<const std::byte*>(&pool_[0].storage);
        const auto* pool_end = reinterpret_cast<const std::byte*>(&pool_[PoolSize - 1].storage) + sizeof(T);
    ...
    friend struct TrieNodePoolDeleter;  // Allow trie deleter to access deallocate_raw
