...
class CatToken {
 public:
  ...
  /**
   * @brief Factory method for creating validated tokens with memory pool optimization
   */
  template<typename CoreClaims_T = CoreClaims, typename CatClaims_T = CatClaims,
           typename InfoClaims_T = InformationalClaims, typename DpopClaims_T = DpopClaims,
           typename ReqClaims_T = RequestClaims, typename CompClaims_T = CompositeClaims>
  static auto createValidated(CoreClaims_T&& core_claims, CatClaims_T&& cat_claims = {},
  ...
  /**
   * @brief Factory method for creating validated tokens without memory pool (for stack allocation)
   */
  template<typename CoreClaims_T = CoreClaims, typename CatClaims_T = CatClaims,
           typename InfoClaims_T = InformationalClaims, typename DpopClaims_T = DpopClaims,
           typename ReqClaims_T = RequestClaims, typename CompClaims_T = CompositeClaims>
  static CatToken createValidatedStack(CoreClaims_T&& core_claims, CatClaims_T&& cat_claims = {},
                                      InfoClaims_T&& info_claims = {}, DpopClaims_T&& dpop_claims = {},
                                      ReqClaims_T&& req_claims = {}, CompClaims_T&& comp_claims = {}) 
    requires std::constructible_from<CoreClaims, CoreClaims_T> && 
             std::constructible_from<CatClaims, CatClaims_T> &&
  ...
  void validateTokenStructure() const {
    ...
    // Note: Time relationship validation (EXP vs NBF) is handled by CatTokenValidator
    // during validation, not during construction
    
    // Additional bounds checking for time values
    // TODO: Revisit these limits based on real-world usage
    constexpr int64_t MAX_TIMESTAMP = 32503680000; // Year 3000 (more generous for testing)
    constexpr int64_t MIN_TIMESTAMP = 946684800;   // Year 2000
    
    if (core.exp.has_value()) {
      if (core.exp.value() > MAX_TIMESTAMP || core.exp.value() < MIN_TIMESTAMP) {
        throw InvalidClaimValueError("Expiration time is outside reasonable bounds");
      }
    }

    if (core.nbf.has_value()) {
      if (core.nbf.value() > MAX_TIMESTAMP || core.nbf.value() < MIN_TIMESTAMP) {
        throw InvalidClaimValueError("Not-before time is outside reasonable bounds");
      }
    }
    ...
    if (cat.catu.has_value()) {
      ...
      if (cat.catu.value() > MAX_USAGE_LIMIT) {
        throw InvalidClaimValueError("Usage limit exceeds maximum allowed value");
      }
    }
...
namespace claim_validation {
  ...
  using StandardClaimRegistry = ClaimRegistry<
    IssuerClaim, AudienceClaim, SubjectClaim, ExpirationClaim, 
    NotBeforeClaim, IssuedAtClaim, CwtIdClaim, ConfirmationClaim,
    CatReplayClaim, CatProofClaim, CatVersionClaim, CatNetworkInterfacesClaim,
    CatUsageClaim, CatMethodsClaim, CatAlpnClaim, CatHostsClaim,
    CatGeoIsoClaim, CatGeoCoordClaim, GeohashClaim, CatGeoAltitudeClaim,
    CatTokenPublicKeyClaim, CatInterfaceDataClaim, CatDpopClaim,
    CatInterfaceClaim, CatRequestClaim, OrClaim, NorClaim, AndClaim,
    MoqtClaim, MoqtRevalidationClaim
  >;
}
...
CatToken& CatToken::withMoqtActionsDynamic(const ActionRange& actions, 
  ...
  moqt_claims.addScope(actions, std::move(namespace_match), std::move(track_match));
  return *this;
}
...
CatToken& CatToken::withMoqtActions(MoqtBinaryMatch namespace_match,
                                            MoqtBinaryMatch track_match) {
  auto& moqt_claims = extended.getMoqtClaims();
  moqt_claims.template addCompileTimeScope<Actions...>(std::move(namespace_match), std::move(track_match));
  return *this;
}
