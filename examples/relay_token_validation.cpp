...
int main() {
    ...
    std::vector<uint8_t> cwt_bytes = cwt.createCwt(CwtMode::Signed, issuer_signer);
    ...
    std::cout << "  (simulating network receive of " << cwt_bytes.size() << " bytes)\n\n";
    ...
    std::cout << "  Token issuer: " << cwt_result.token->core.iss.value_or("unknown") << "\n\n";
