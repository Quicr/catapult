 ...
 * @brief Example demonstrating JSON serialization of CAT tokens with composite
 * claims
 *
...
CatToken create_moqt_token() {
  ...
  moqt_claims.addScope(publish_actions, MoqtBinaryMatch::exact("live-streams"),
                       MoqtBinaryMatch::prefix("user123-"));

  // Subscriber permissions
  std::vector<int> subscribe_actions = {moqt_actions::SUBSCRIBE,
                                        moqt_actions::FETCH};
  moqt_claims.addScope(subscribe_actions,
...
CatToken create_composite_token() {
  ...
  or_claim_sets.emplace_back(
      std::make_unique<CatToken>(std::move(publisher_token)));
  or_claim_sets.emplace_back(
      std::make_unique<CatToken>(std::move(moderator_token)));
  ...
  and_claim_sets.emplace_back(
      std::make_unique<CatToken>(std::move(admin_token)));
...
CatToken create_nested_composite_token() {
  ...
  nor_claim_sets.emplace_back(
      std::make_unique<CatToken>(std::move(blocked_token)));
 ...
 */
void output_token(const CatToken& token, bool pretty_print, int indent,
                  bool base64_output) {
  if (base64_output) {
...
void process_base64_to_json(const std::string& base64_input) {
  try {
    ...
    std::cout << "Input Base64: " << base64_input.substr(0, 80)
              << (base64_input.length() > 80 ? "..." : "") << "\n\n";

    // Decode base64 to JSON
    auto json_output =
        json_serialization::base64_utils::base64_to_json(base64_input);
    ...
    std::cout << "Token issuer: "
              ...
              << "\n";
    std::cout << "Token expiry: "
              << (token.core.exp.has_value() ? std::to_string(*token.core.exp)
                                             : "(none)")
              << "\n\n";
...
void process_json_to_base64(const std::string& json_input) {
  try {
    ...
    std::cout << "Input JSON: " << json_input.substr(0, 200)
              << (json_input.length() > 200 ? "..." : "") << "\n\n";

    // Convert JSON to base64
    auto base64_output =
        json_serialization::base64_utils::json_to_base64(json_input);
...
void show_claim_type_examples() {
  ...
  geo_token.core.exp = std::chrono::system_clock::to_time_t(
      std::chrono::system_clock::now() + std::chrono::hours{1});
  ...
  std::cout << "Base64: "
            << json_serialization::to_base64_json(geo_token, false) << "\n";
  ...
  std::cout << "Base64: "
            << json_serialization::to_base64_json(moqt_token, false) << "\n";
...
void print_usage(const char* program_name) {
  ...
  std::cout
      << "  --indent N       Pretty print with N spaces indent (default: 2)\n";
  ...
  std::cout << "  " << program_name
            << " --base64       # Base64-encoded JSON output\n";
  std::cout << "  " << program_name
            << " --examples     # Show examples by claim type\n";
}
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      ...
      try {
        ...
        std::cerr << "Error: Invalid indent value: " << argv[i + 1]
                  << std::endl;
        return 1;
      }
    } else {
