 ...
 * @brief Example demonstrating CWT crypto operations with CAT tokens and
 * composite claims
 *
...
CatToken create_composite_cat_token() {
  ...
  service_token.core.aud =
      std::vector<std::string>{"service-endpoint.example.com"};
  ...
  or_claim_sets.emplace_back(
      std::make_unique<CatToken>(std::move(service_token)));
...
void perform_hmac_operation(const CatToken& token, bool single_key) {
  ...
  try {
    ...
    std::cout << "HMAC Key 1: "
              << bytes_to_hex(std::vector<uint8_t>(key1.begin(), key1.end()))
              << "\n";
    ...
    if (single_key) {
      ...
      std::cout << "Single MAC CWT (Hex): "
                << bytes_to_hex(base64UrlDecode(mac_cwt)) << "\n";
      ...
      // Multiple MAC (for demonstration, we'll create multiple CWTs with
      // different keys)
      auto key2 = HmacSha256Algorithm::generateSecureKey();
      std::cout << "HMAC Key 2: "
                << bytes_to_hex(std::vector<uint8_t>(key2.begin(), key2.end()))
                << "\n";
      ...
      std::cout << "Multiple MAC CWT 1 (Hex): "
                << bytes_to_hex(base64UrlDecode(mac_cwt1)) << "\n";
      std::cout << "Multiple MAC CWT 2 (Base64): " << mac_cwt2 << "\n";
      std::cout << "Multiple MAC CWT 2 (Hex): "
                << bytes_to_hex(base64UrlDecode(mac_cwt2)) << "\n";
...
void perform_sign_operation(const CatToken& token, bool single_signer) {
  ...
  try {
    ...
    std::cout << "ES256 Private Key 1: "
              << bytes_to_hex(std::vector<uint8_t>(private_key1.begin(),
                                                   private_key1.end()))
              << "\n";
    ...
    if (single_signer) {
      ...
      std::cout << "Single Signature CWT (Hex): "
                << bytes_to_hex(base64UrlDecode(signed_cwt)) << "\n";
      ...
      // Multiple signatures (for demonstration, we'll create multiple CWTs with
      // different keys)
      auto [private_key2, public_key2] =
          Es256Algorithm::generateSecureKeyPair();
      std::cout << "ES256 Private Key 2: "
                << bytes_to_hex(std::vector<uint8_t>(private_key2.begin(),
                                                     private_key2.end()))
                << "\n";
      ...
      std::cout << "Multiple Signature CWT 1 (Hex): "
                << bytes_to_hex(base64UrlDecode(signed_cwt1)) << "\n";
      std::cout << "Multiple Signature CWT 2 (Base64): " << signed_cwt2 << "\n";
      std::cout << "Multiple Signature CWT 2 (Hex): "
                << bytes_to_hex(base64UrlDecode(signed_cwt2)) << "\n";
...
void perform_encrypt_operation(const CatToken& token, bool single_recipient) {
  ...
  try {
    ...
    if (single_recipient) {
      // Single recipient
      std::string encrypted_cwt =
          cwt.createCwtBase64(CwtMode::Encrypted, aes_algo);
      std::cout << "Single Recipient Encrypted CWT (Base64): " << encrypted_cwt
                << "\n";
      std::cout << "Single Recipient Encrypted CWT (Hex): "
                << bytes_to_hex(base64UrlDecode(encrypted_cwt)) << "\n";

      // Decrypt
      auto decrypted_cwt = Cwt::validateCwtBase64(encrypted_cwt, aes_algo);
      std::cout << "Decryption: SUCCESS\n";
      std::cout << "Decrypted Token Issuer: "
                << (decrypted_cwt.payload.core.iss.has_value()
                        ? *decrypted_cwt.payload.core.iss
                        : "none")
                << "\n";
    } else {
      // Multiple recipients (for demonstration, we'll create multiple CWTs with
      // different keys)
      ...
      std::string encrypted_cwt1 =
          cwt.createCwtBase64(CwtMode::Encrypted, aes_algo);
      std::string encrypted_cwt2 =
          cwt.createCwtBase64(CwtMode::Encrypted, aes_algo2);

      std::cout << "Multiple Recipients Encrypted CWT 1 (Base64): "
                << encrypted_cwt1 << "\n";
      std::cout << "Multiple Recipients Encrypted CWT 1 (Hex): "
                << bytes_to_hex(base64UrlDecode(encrypted_cwt1)) << "\n";
      std::cout << "Multiple Recipients Encrypted CWT 2 (Base64): "
                << encrypted_cwt2 << "\n";
      std::cout << "Multiple Recipients Encrypted CWT 2 (Hex): "
                << bytes_to_hex(base64UrlDecode(encrypted_cwt2)) << "\n";
...
void print_usage(const char* program_name) {
  ...
  std::cout << "  " << program_name
            << " --mac --single        # Single MAC operation\n";
  std::cout << "  " << program_name
            << " --sign --multiple     # Multiple signature operation\n";
  std::cout << "  " << program_name
            << " --encrypt             # Single encryption operation\n";
  std::cout
      << "  " << program_name
      << " --all --single        # All operations with single recipients\n";
}
...
int main(int argc, char* argv[]) {
  ...
  if (!perform_mac && !perform_sign && !perform_encrypt && !perform_all) {
    std::cout << "No operation specified. Use --help for options or --all to "
  ...
  std::cout << "=================================\n";
  std::cout << "Mode: " << (single_mode ? "Single" : "Multiple")
            << " recipient/key\n";
  ...
  try {
    ...
    std::cout << "Token:\n";
    std::cout << "Issuer: "
              << (token.core.iss.has_value() ? *token.core.iss : "none")
              << "\n";
    std::cout << "Audience: ";
