 ...
 *
 * This example shows how to create CWTs with multiple signatures using
 * different algorithms and validate them with the per-signature validation
 * method.
 */
...
int main() {
  try {
    ...
    for (size_t i = 0; i < cwt.signatures.size(); ++i) {
      CAT_LOG_INFO("  Signature {}: Algorithm ID {}", i,
                   cwt.signatures[i].algorithmId);
    }

    // Create the multi-signed CWT using COSE_Sign format and use ES256 as
    // primary
    std::string multiSignedCwt =
        cwt.createCwtBase64(CwtMode::MultiSigned, *es256Algorithm);
    ...
    Cwt validatedCwt =
        Cwt::validateMultiSignedCwtBase64(multiSignedCwt, algorithmMap);

    CAT_LOG_INFO("Validated signatures count: {}",
                 validatedCwt.signatures.size());
    CAT_LOG_INFO("Decoded issuer: {}",
                 validatedCwt.payload.core.iss.value_or("none"));
    ...
    for (size_t i = 0; i < validatedCwt.signatures.size(); ++i) {
      ...
      CAT_LOG_INFO("  Signature {}: {} (ID: {})", i, algName,
                   validatedCwt.signatures[i].algorithmId);
    }
