...
TEST_SUITE("RFC 8152 COSE Structure Compliance") {
  TEST_CASE("RFC 8152 Section 4.4 - COSE_Sign1 Sig_structure Format") {
    std::vector<uint8_t> protectedHeader = {0x43, 0xa1, 0x01,
                                            0x26};  // CBOR: {1: -7} (ES256)
    std::vector<uint8_t> payload = {0x48, 0x65, 0x6c,
                                    0x6c, 0x6f, 0x21};  // "Hello!"
    std::vector<uint8_t> externalAAD = {};              // Empty for this test

    auto sigStructure =
        createCoseSign1Input(protectedHeader, payload, externalAAD);

    struct cbor_load_result result;
    cbor_item_t* item =
        cbor_load(sigStructure.data(), sigStructure.size(), &result);
    ...
    std::string context(
        reinterpret_cast<const char*>(cbor_string_handle(array[0])),
  ...
  TEST_CASE("RFC 8152 Section 4.4 - COSE_Sign1 with External AAD") {
    ...
    std::vector<uint8_t> payload = {0x48, 0x65, 0x6c,
                                    0x6c, 0x6f, 0x21};  // "Hello!"
    std::vector<uint8_t> externalAAD = {0x57, 0x6F, 0x72, 0x6C,
                                        0x64};  // "World"
  ...
  TEST_CASE("RFC 8152 Section 4.4 - COSE_Signature Sig_structure Format") {
    ...
    auto sigStructure = createCoseSignInput(
        protectedHeader, signatureProtectedHeader, externalAAD, payload);
...
TEST_SUITE("CWT Multi-Signature (COSE_Sign) Tests") {
  TEST_CASE(
      "RFC 8392 Section 7.1 - COSE_Sign Creation with Multiple Signatures") {
    CatToken token;
    ...
    std::vector<uint8_t> testKey(
        reinterpret_cast<const uint8_t*>("test-key-16-bytes"),
    ...
    std::string multiSignedCwt =
        cwt.createCwtBase64(CwtMode::MultiSigned, *hmac);
    ...
    Cwt validatedCwt =
        Cwt::validateMultiSignedCwtBase64(multiSignedCwt, algorithms);
  ...
  TEST_CASE("RFC 8392 Section 7.2 - COSE_Sign Validation") {
    ...
    std::string cwtString =
        cwt.createCwtBase64(CwtMode::MultiSigned, *algorithm);
  ...
  TEST_CASE("COSE_Sign vs COSE_Sign1 Structure Validation") {
    ...
    std::string singleSigCwt =
        singleCwt.createCwtBase64(CwtMode::Signed, *algorithm);

    // Create COSE_Sign (multi signature)
    Cwt multiCwt(algorithm->algorithmId(), token);
    multiCwt.addSignature(*algorithm);
    std::string multiSigCwt =
        multiCwt.createCwtBase64(CwtMode::MultiSigned, *algorithm);
    ...
    algorithms.emplace(algorithm->algorithmId(), std::cref(*algorithm));
    Cwt validatedMulti =
        Cwt::validateMultiSignedCwtBase64(multiSigCwt, algorithms);

    // Check structure differences
    CHECK(validatedSingle.signatures.size() ==
          0);  // COSE_Sign1 doesn't populate signatures array
  ...
  TEST_CASE("Multiple Algorithms in COSE_Sign") {
    ...
    std::vector<uint8_t> multiAlgKey(
        reinterpret_cast<const uint8_t*>("multi-alg-key-16b"),
  ...
  TEST_CASE("Error Cases - COSE_Sign") {
    ...
    SUBCASE("No signatures for MultiSigned mode") {
      ...
      CHECK_THROWS_AS(cwt.createCwtBase64(CwtMode::MultiSigned, *algorithm),
                      CryptoError);
    }
  ...
  TEST_CASE("Performance - Large Number of Signatures") {
    ...
    std::vector<uint8_t> perfKey(
        reinterpret_cast<const uint8_t*>("perf-test-key-16"),
...
TEST_SUITE("CWT Signing Integration Tests") {
  TEST_CASE("End-to-End Multi-Signature Workflow") {
    ...
    std::vector<uint8_t> sharedKey(
        reinterpret_cast<const uint8_t*>("shared-secret-16b"),
    ...
    std::string corporateToken =
        corporateCwt.createCwtBase64(CwtMode::MultiSigned, *caAlgorithm);
    ...
    Cwt caValidated =
        Cwt::validateMultiSignedCwtBase64(corporateToken, algorithms);
  ...
  TEST_CASE("Single vs Multi-Signature Mode Comparison") {
    ...
    algorithms.emplace(algorithm->algorithmId(), std::cref(*algorithm));
    Cwt validatedMulti =
        Cwt::validateMultiSignedCwtBase64(multiToken, algorithms);
...
TEST_SUITE("Cwt::decodeHeader") {
  ...
  TEST_CASE("Throws on invalid CBOR") {
    ...
    CHECK_THROWS_AS(Cwt::decodeHeader(std::span<const uint8_t>(garbage.data(),
                                                               garbage.size())),
                    InvalidCborError);
  }
  ...
  TEST_CASE("Throws on non-array CBOR") {
    ...
    CHECK_THROWS_AS(Cwt::decodeHeader(std::span<const uint8_t>(
                        notArray.data(), notArray.size())),
