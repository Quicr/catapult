...
struct MediumObject {
  ...
  MediumObject(uint32_t i = 0) : id(i) { data.fill(i); }
};
...
struct LargeObject {
  ...
  LargeObject(uint32_t i = 0) : id(i) { data.fill(i); }
};
...
BENCHMARK(BM_Pool_Multithreaded)
...
static void BM_Standard_Multithreaded(benchmark::State& state) {
  for (auto _ : state) {
    ...
    for (int i = 0; i < 50; ++i) {
      ptrs.push_back(
          std::make_unique<SmallObject>(state.thread_index() * 1000 + i, 3.14));
    }
