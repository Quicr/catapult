...
std::vector<std::string> generateRandomStrings(size_t count, size_t minLength,
                                               size_t maxLength) {
...
std::vector<std::string> generatePrefixPatterns() {
  return {"http://", "https://", "ftp://", "file://", "api/v1/",
          "api/v2/", "api/",     "user/",  "admin/",  "guest/",
          "en",      "fr",       "de",     "es",      "it"};
}

std::vector<std::string> generateSuffixPatterns() {
  return {".com",   ".org",     ".net",  ".edu", ".gov",   ".html",
          ".css",   ".js",      ".png",  ".jpg", "/index", "/home",
          "/about", "/contact", "admin", "user", "guest"};
}

std::vector<std::string> generateTestTexts() {
  return {"https://example.com/api/v1/users/admin",
...
static void BM_PrefixTrie_SearchPrefix_Miss(benchmark::State& state) {
  PrefixTrie trie;
  auto patterns = generatePrefixPatterns();
  std::vector<std::string> missTexts = {"xyz://invalid", "unknown/path",
                                        "random.text"};
...
static void BM_SuffixTrie_SearchSuffix_Miss(benchmark::State& state) {
  SuffixTrie trie;
  auto patterns = generateSuffixPatterns();
  std::vector<std::string> missTexts = {"invalidxyz", "pathunknown",
                                        "textrandom"};
