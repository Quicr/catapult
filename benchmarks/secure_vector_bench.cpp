...
static void BM_ConstantTimeCompare(benchmark::State& state) {
  ...
  for (auto _ : state) {
    int result =
        secure_utils::constantTimeCompare(data1.data(), data2.data(), size);
...
BENCHMARK(BM_SecureVector_Allocation)
    ->Range(1024, 1024 * 1024)
    ->Unit(benchmark::kMicrosecond);
BENCHMARK(BM_RegularVector_Allocation)
    ->Range(1024, 1024 * 1024)
    ->Unit(benchmark::kMicrosecond);

BENCHMARK(BM_SecureVector_Copy)
    ->Range(1024, 1024 * 1024)
    ->Unit(benchmark::kMicrosecond);
BENCHMARK(BM_RegularVector_Copy)
    ->Range(1024, 1024 * 1024)
    ->Unit(benchmark::kMicrosecond);

BENCHMARK(BM_ConstantTimeCompare)
    ->Range(32, 1024 * 16)
    ->Unit(benchmark::kNanosecond);
BENCHMARK(BM_RegularMemcmp)->Range(32, 1024 * 16)->Unit(benchmark::kNanosecond);

BENCHMARK(BM_ToSecureVector)
    ->Range(1024, 1024 * 1024)
    ->Unit(benchmark::kMicrosecond);
BENCHMARK(BM_ToRegularVector)
    ->Range(1024, 1024 * 1024)
    ->Unit(benchmark::kMicrosecond);

BENCHMARK(BM_SecureVector_PushBack)
    ->Range(1024, 1024 * 64)
    ->Unit(benchmark::kMicrosecond);
BENCHMARK(BM_RegularVector_PushBack)
    ->Range(1024, 1024 * 64)
    ->Unit(benchmark::kMicrosecond);